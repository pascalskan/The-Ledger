...
test('NC-22: Mark Read action removes unread highlight and shows toast', async ({ page }) => {
  ...
  // Scope to the aria-live toast region to avoid strict-mode collision with
  // KPI / status badges that also contain the word "read".
  await expect(
...
test('NC-23: Dismiss action shows toast and audit confirmation', async ({ page }) => {
  ...
  // Scope to the aria-live toast region to avoid strict-mode collision with
  // status badges and KPI cards that also contain the word "dismissed".
  await expect(
...
test('NC-26: Bell dropdown opens and shows preview notifications', async ({ page }) => {
  ...
  // Use .first() to avoid strict-mode failure when .or() resolves to multiple
  // elements (both notif-001 and notif-003 may be in the DOM simultaneously).
  await expect(
