 ...
 * These tests verify the priority queue, categories, ordering, filtering,
 * insights and detail rendering, and that the approval workflow is unchanged.
 */
...
test('REV-PRI-03 Priority queue rows render scores and categories in order', async ({ page }) => {
  ...
  // At least one priority badge is shown in the queue.
  await expect(
...
test('REV-PRI-06 Standard / Priority order toggle reorders the job table', async ({ page }) => {
  ...
  // Standard order is the default.
  await expect(page.getByTestId('review-order-standard')).toHaveAttribute(
    'aria-pressed',
    'true'
  );

  // Switching to Priority order updates the pressed state (visibility change only).
  await page.getByTestId('review-order-priority').click();
  await expect(page.getByTestId('review-order-priority')).toHaveAttribute(
    'aria-pressed',
    'true'
  );
  // The job table remains present after reordering.
...
test('REV-PRI-07 Job rows carry priority badges', async ({ page }) => {
  ...
  await expect(
    jobPriority.first().locator('[data-testid^="priority-badge-"]')
  ).toBeVisible();
});
...
test('REV-PRI-10 Prioritisation is visibility-only — no approve/reject controls in the panel', async ({ page }) => {
  ...
  // Doctrine: the priority layer must not expose approval actions.
  await expect(panel.getByRole('button', { name: /approve|reject/i })).toHaveCount(0);
});

test('REV-PRI-11 Approval workflow remains intact after prioritisation', async ({ page }) => {
  await loginAsCEO(page);
  await openReviewCenter(page);
  await page.getByRole('button', { name: /Review Items/i }).first().click();
  // The existing approve / reject controls are still present and usable.
