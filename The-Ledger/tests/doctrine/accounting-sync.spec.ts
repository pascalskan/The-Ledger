 ...
 *
 * All tests use:
 *   - soft-login pattern (no page.goto on auth)
 *   - clearBrowserState in beforeEach
 *   - data-testid selectors
 */

import { test, expect } from "@playwright/test";
import { softLoginCEO } from "../helpers/login";
...
test("1. Accounting Sync tab trigger renders in Financial Explorer", async ({ page }) => {
  ...
  await softLoginCEO(page);
  await navigateTo(page, "/financial-explorer");
...
test("5. Sync queue table renders with seeded records", async ({ page }) => {
  ...
  // Confirm at least the known seeded rows exist
...
test("6. Provider badges are visible in the sync queue", async ({ page }) => {
  ...
  await navigateTo(page, "/financial-explorer");
  await page.getByTestId("tab-accounting-sync").click();
  // Multiple provider badges for quickbooks should exist (sync-001, sync-003, sync-005)
...
test("12. Sync audit trail toggle opens the audit trail panel", async ({ page }) => {
  ...
  // Seeded audit entries (audit-sync-001 through audit-sync-004) should appear
...
test("13. Job Detail page shows Accounting Synchronization panel for seeded job", async ({ page }) => {
  await clearBrowserState(page);
  await softLoginCEO(page);
  await navigateTo(page, `/jobs/${SEEDED_JOB_ID}`);
