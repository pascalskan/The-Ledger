 ...
 *
 * This phase MEASURES decision-making; it never alters it. These tests verify
 * KPIs, throughput, approval performance, bottlenecks, reviewer analytics,
 * type analytics, financial throughput, health score and insights render, and
 * that the analytics surface is read-only and review workflows are unchanged.
 */
...
test('REV-AN-01 CEO review page shows the analytics dashboard', async ({ page }) => {
...
test('REV-AN-04 Bottleneck analysis renders with warnings/lists', async ({ page }) => {
  await loginAsCEO(page);
  await openReviewCenter(page);
  const b = page.getByTestId('review-bottlenecks');
...
test('REV-AN-10 Analytics is read-only and PMs do not see it; workflows intact', async ({ page }) => {
  ...
  // Existing single-item workflow remains.
