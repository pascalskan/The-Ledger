 ...
 * They must never assert that any approval happens automatically — the
 * dashboard creates, approves, rejects and corrects nothing.
 */
...
test('REV-EXE-01 CEO sees the executive review dashboard above the queue', async ({ page }) => {
  await loginAsCEO(page);
  await openReviewCenter(page);
  await expect(page.getByTestId('review-executive-dashboard')).toBeVisible();
  // The legacy job queue is preserved beneath it.
...
test('REV-EXE-04 Requires-attention queue renders prioritised rows', async ({ page }) => {
  ...
  // Seed has pending reviews, so at least one attention row is shown.
...
test('REV-EXE-08 PMs do not see the CEO executive dashboard', async ({ page }) => {
  ...
  await expect(page.getByText('Jobs Requiring Review')).toBeVisible();
  // ...but the CEO-only executive layer is not rendered.
...
test('REV-EXE-09 Review detail surfaces read-only executive context', async ({ page }) => {
  await loginAsCEO(page);
  await openReviewCenter(page);
  // Enter the first job that requires review.
...
test('REV-EXE-10 Dashboard is visibility-only — no approve/reject controls in it', async ({ page }) => {
  ...
  // The executive dashboard must not expose approval actions (Approval Doctrine).
