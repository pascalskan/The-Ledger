...
export function OperationalDrawer(props: OperationalDrawerProps) {
  ...
  return (
    <div className="w-80 lg:w-96 bg-card border-l border-border h-full flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] transition-transform duration-300 transform translate-x-0">
      <div className="p-4 border-b border-border flex items-center justify-between bg-muted/50">
        <div>
          <h3 className="font-semibold text-foreground">{type === 'job' ? 'Job Analysis' : 'Daily Rollup'}</h3>
          <p className="text-xs text-muted-foreground">{type === 'job' ? selectedItem.title : selectedItem.dateStr}</p>
        </div>
        ...
        {type === 'job' ? (
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-muted rounded border border-border">
                  <div className="text-[10px] text-muted-foreground mb-1">Contract Value</div>
                  <div className="font-semibold text-foreground">{formatCur(selectedItem.contractValue)}</div>
                </div>
                <div className="p-3 bg-muted rounded border border-border">
                  <div className="text-[10px] text-muted-foreground mb-1">Cost to Date</div>
                  <div className="font-semibold text-foreground">{formatCur(selectedItem.costToDate)}</div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className={`p-4 rounded-lg border ${
                ...
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-foreground">Internal Forecast</span>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between items-center py-1.5 border-b border-border">
                  <span className="text-muted-foreground">Remaining Labor</span>
                  <span className="font-medium text-foreground">{selectedItem.remainingLaborHours} hrs</span>
                </div>
                <div className="flex justify-between items-center py-1.5 border-b border-border">
                  <span className="text-muted-foreground">Assigned Crew</span>
                  <span className="font-medium text-foreground">{selectedItem.crewCount} Personnel</span>
                </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div className="p-3 bg-muted rounded border border-border">
                <div className="text-[10px] text-muted-foreground uppercase font-semibold mb-1 tracking-wider">Est. Labor Cost</div>
                <div className="text-lg font-bold text-foreground">{formatCur(selectedItem.totalEstimatedLaborCost)}</div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-muted-foreground font-medium">Crew Allocation</span>
                  <span className="text-foreground font-bold">{selectedItem.crewAllocationPct}%</span>
                </div>
