...
const STATUS_META: Record<ClientMilestoneStatus, { icon: typeof Flag; cls: string }> = {
  ...
  Upcoming: { icon: Clock, cls: "bg-muted text-muted-foreground border-border" },
...
export function ProjectMilestones({ milestones }: { milestones: PortalMilestone[] }) {
  return (
    <Card className="border-border" data-testid="portal-milestones">
      <CardHeader className="pb-3 border-b border-border">
        <CardTitle className="text-lg flex items-center gap-2">
          <Flag className="h-5 w-5 text-muted-foreground" /> Milestones
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-4">
        {milestones.length === 0 ? (
          <p className="text-sm text-muted-foreground italic" data-testid="portal-milestones-empty">No milestones yet.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div className="h-8 w-8 rounded-full bg-muted border border-border flex items-center justify-center shrink-0">
                    <Icon className={`h-4 w-4 ${m.status === "In Progress" ? "animate-spin-slow" : ""} text-muted-foreground`} />
                  </div>
