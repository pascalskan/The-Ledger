...
const STATUS_META: Record<ClientDeliverableStatus, { icon: typeof FileText; cls: string }> = {
  ...
  Pending: { icon: FileClock, cls: "bg-slate-100 text-slate-600 border-slate-200" },
};

export function ProjectDeliverables({ deliverables }: { deliverables: PortalDeliverable[] }) {
  return (
    <Card className="border-slate-200" data-testid="portal-deliverables">
      <CardHeader className="pb-3 border-b border-slate-100">
        <CardTitle className="text-lg flex items-center gap-2">
          <FileCheck2 className="h-5 w-5 text-slate-500" /> Deliverables
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-4">
        {deliverables.length === 0 ? (
          <p className="text-sm text-slate-500 italic" data-testid="portal-deliverables-empty">
