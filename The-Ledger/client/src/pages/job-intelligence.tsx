...
import { TrendingUp, AlertTriangle, AlertCircle, CheckCircle2, ArrowRight } from "lucide-react";
...
export default function JobIntelligenceDashboard() {
  ...
  const activeMetrics = allMetrics.filter(m => m.job.status === "Active" || m.job.status === "Planned");
      ...
      <div className="space-y-8 pb-12">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Operational analytics derived from live QuickBooks financial data.
          </p>
        </div>

        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {activeMetrics.map((metrics) => {
            const { job, contractValue, totalCost, profit, margin, burn, healthScore, risks, invoicedAmount, paidAmount } = metrics;
            ...
            return (
              <Card key={job.id} className="flex flex-col overflow-hidden transition-shadow hover:shadow-md">
                <CardHeader className="pb-4 border-b bg-slate-50/50">
                  <div className="flex justify-between items-start mb-2">
                    <Badge variant={job.status === "Active" ? "default" : "secondary"}>
                      {job.status}
                    </Badge>
                    <div className={cn("px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1.5", healthColor)}>
                      {healthScore >= 80 ? <CheckCircle2 className="w-3.5 h-3.5" /> : healthScore < 60 ? <AlertCircle className="w-3.5 h-3.5" /> : <AlertTriangle className="w-3.5 h-3.5" />}
                      Health: {healthScore}
                    </div>
                  </div>
                ...
                <CardContent className="p-5 flex-1 flex flex-col gap-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-bold text-lg">${contractValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-semibold">${totalCost.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Est. Margin</p>
                      <p className={cn("font-semibold flex items-center gap-1", margin >= 20 ? "text-green-600" : margin > 0 ? "text-yellow-600" : "text-red-600")}>
                  ...
                  <div className="space-y-1.5 mt-auto">
                    <div className="flex justify-between text-xs">
                      <span className="font-medium text-slate-700">Budget Burn</span>
                      <span className={cn("font-bold", burn > 95 ? "text-red-600" : burn > 85 ? "text-yellow-600" : "text-slate-600")}>
                        {burn.toFixed(1)}%
                      </span>
                    </div>
                    <Progress value={burn} className={cn("h-2", burn > 95 ? "[&>div]:bg-red-500" : burn > 85 ? "[&>div]:bg-yellow-500" : "")} />
                  </div>
                  ...
                  <div className="bg-slate-50 p-3 rounded-lg text-xs flex justify-between items-center border">
                    <div>
                      ...
                      <p className="font-medium">${invoicedAmount.toLocaleString()}</p>
                    </div>
        ...
        {activeMetrics.length === 0 && (
          <div className="text-center py-12 border-2 border-dashed rounded-xl border-slate-200">
            ...
            <p className="text-sm text-slate-500 mt-1">There are no active jobs to display intelligence for.</p>
          </div>
        )}
      </div>
    </Layout>
