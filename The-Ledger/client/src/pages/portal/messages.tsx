    ...
    <div className="space-y-6" data-testid="portal-messages">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Project conversations with your delivery team.</p>
        </div>
      ...
      {composing && (
        <Card className="border-border" data-testid="portal-thread-compose">
      ...
      {threads.length === 0 ? (
        <div className="py-12 text-center border-2 border-dashed border-border rounded-lg bg-card" data-testid="portal-messages-empty">
          <MessagesSquare className="h-8 w-8 mx-auto text-muted-foreground mb-3" />
        ...
        <div className="border border-border rounded-lg bg-card divide-y divide-slate-100" data-testid="portal-thread-list">
          {threads.map((t) => (
            <button
              key={t.id}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  ...
                  <span className="text-[11px] text-muted-foreground">{new Date(t.updatedAt).toLocaleDateString()}</span>
                </div>
              </div>
            </button>
          ))}
...
const SENDER_META = {
  ...
  ProjectManager: { icon: UserCog, label: "Project Manager", cls: "bg-muted text-muted-foreground" },
  System: { icon: Settings2, label: "System", cls: "bg-muted text-muted-foreground" },
} as const;
...
function ThreadDetail({
  ...
  return (
    <div className="space-y-6" data-testid="portal-thread-detail">
      <Button variant="ghost" size="sm" onClick={onBack} className="-ml-3 text-muted-foreground hover:text-foreground" data-testid="portal-thread-back">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-foreground" data-testid="portal-thread-detail-subject">
          {thread.subject}
        </h1>
        <p className="text-muted-foreground text-sm mt-1">{thread.projectTitle}</p>
      </div>
      ...
      <Card className="border-border">
        ...
        <CardContent className="pt-4">
            ...
            {messages.map((m) => {
              ...
              return (
                <li key={m.id} className="py-4 flex gap-3" data-testid={`portal-message-${m.id}`}>
                  <div className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 border border-border ${meta.cls}`}>
                    <Icon className="h-4 w-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <span className="text-sm font-medium text-foreground" data-testid={`portal-message-sender-${m.id}`}>
                    ...
                    <p className="text-sm text-foreground mt-1 leading-relaxed">{m.message}</p>
                  </div>
                </li>
              );
            })}
          ...
          {thread.status === "Closed" ? (
            <div className="mt-4 pt-4 border-t border-border text-sm text-muted-foreground italic" data-testid="portal-thread-closed-note">
            ...
            <div className="mt-4 pt-4 border-t border-border space-y-2">
              ...
              <div className="flex gap-2">
                <textarea
                  ...
                  className="min-h-[72px] flex-1 rounded-md border border-border bg-card px-3 py-2 text-sm"
                  value={reply}
